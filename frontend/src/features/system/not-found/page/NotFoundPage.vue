...
const requestedPath = computed(() => {
  return route.fullPath.length > 0
    ? route.fullPath
    : i18n.t("notFound:requestedPathUnknown");
});
</script>

<template>
  <section class="w-full" :aria-label="$t('notFound:ariaLabel')">
    <div class="mx-auto max-w-5xl px-6 py-12">
      <div class="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
        <div class="flex flex-col gap-8 md:flex-row md:items-start md:justify-between">
          ...
          <aside class="w-full max-w-md" :aria-label="$t('notFound:asideAriaLabel')">
            <div class="rounded-2xl border border-slate-200 bg-slate-50 p-6">
              <h2 class="text-sm font-semibold text-slate-900">
                {{ $t("notFound:whatYouCanDoTitle") }}
              </h2>
