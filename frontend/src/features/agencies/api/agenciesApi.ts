...
export const agenciesApi = {
  ...
  async list(
    ...
    return http.get<PagedResultDto<AgencyListItemDto>>(routes.api.agencies.list(), { params });
  },

  // GET /api/agencies/{id}
  async getById(id: string): Promise<AgencyDetailsDto> {
    if (!id) {
      throw new Error('agenciesApi.getById: id is required');
    }

    return http.get<AgencyDetailsDto>(routes.api.agencies.getById(id));
  },
};
