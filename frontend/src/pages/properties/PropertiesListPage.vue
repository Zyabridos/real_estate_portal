...
const data = ref<PagedResult<PropertyListItem> | null>(null);
...
async function load(): Promise<void> {
  ...
  try {
    ...
    const res = await fetch("/api/properties?page=1&pageSize=20");
    ...
    const json = (await res.json()) as PagedResult<PropertyListItem>;
    data.value = json;
    ...
    <div class="w-full px-6 py-2">
      ...
      <div v-else-if="state === 'error'" class="mt-8">
        <div class="rounded-2xl border border-rose-200 bg-rose-50 p-6">
          <div class="text-sm font-semibold text-rose-900">Failed to load properties</div>
