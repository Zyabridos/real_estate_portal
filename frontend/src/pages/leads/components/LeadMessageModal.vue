  ...
  >
    ...
    <template #description>
      <p :id="descId" class="mt-1 text-sm text-slate-600" data-testid="lead-message-subtitle">
        {{ fullName ? `From: ${fullName}` : "Message details" }}
    ...
    <div class="space-y-3" data-testid="lead-message-body">
      <div v-if="isLoading" class="rounded-xl border border-slate-200 bg-white p-4" role="status" data-testid="lead-message-loading">
        <div class="flex items-center gap-3">
          <span class="h-4 w-4 animate-spin rounded-full border-2 border-slate-300 border-t-slate-900" aria-hidden="true" />
          <span class="text-sm text-slate-700">Loading message…</span>
        </div>
      </div>

      <div v-else-if="errorMessage" class="rounded-xl border border-rose-200 bg-rose-50 p-4 text-rose-900" role="alert" data-testid="lead-message-error">
        <p class="text-sm font-medium">{{ errorMessage }}</p>
      </div>

      <div v-else class="rounded-xl border border-slate-200 bg-slate-50 p-4" data-testid="lead-message-content">
        <p class="whitespace-pre-wrap break-words text-sm text-slate-900">
          {{ message && message.trim().length ? message : "— No message provided —" }}
        </p>
      </div>
    </div>
