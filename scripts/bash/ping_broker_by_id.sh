...
echo ""
info "Fetching broker by id..."

if [ $# -lt 1 ] || [ -z "${1:-}" ]; then
  warn "Usage: ping_broker_by_id.sh <guid>"
  highlight "Oops. Seems like you are not using this command correctly."
  highlight "Here is an example: make ping-broker ID=qwertyui-aaaa-bbbb-cccc-abcdefghiqwe"
  exit 1
fi
...
STATUS=""
if ! STATUS="$(curl -sS -o "$OUT" -w "%{http_code}" "$BACKEND_URL/api/brokers/$ID")"; then
